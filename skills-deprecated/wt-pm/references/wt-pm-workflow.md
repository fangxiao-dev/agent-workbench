...
执行约束（Windows/Agent/Codex app 环境）：
- worktree 可以由手动 `git worktree add` 创建，也可以由 Codex app handoff 创建。
- trunk 负责定义任务上下文，不负责执行 task-local 环境准备。
- ignored local files、依赖安装、私有配置生成等副作用应在目标 task worktree 内完成。
- 若任务已有 worktree，继续开发时应直接进入该 worktree，而不是在 trunk 目录里再次 checkout 同一分支。

### Plan

结构化的任务规划，由 task 目录中的三个持久化文件组成：

| 文件 | 用途 |
|------|------|
| `plans/workplans/<task_id>/task_plan.md` | 任务规划：目标、步骤 |
| `plans/workplans/<task_id>/findings.md` | 调研发现：技术决策、风险记录、现有代码分析 |
| `plans/workplans/<task_id>/progress.md` | 执行进度：已完成/进行中/阻塞项/下一步 |
...
│  → commit 规划产出物到 trunk → 准备 branch / worktree handoff   │
└──────────────────────────────┬──────────────────────────────────┘
                               │ enter task worktree
┌──────────────────────────────▼──────────────────────────────────┐
│ Environment 2: Task Worktree — 开发              [SKILL: wt-dev] │
│                                                                  │
│  恢复 plan 上下文 → 初始化本地环境 → Sync trunk → 实现          │
│  → [PAUSE 人工测试] → 再次 Sync trunk + Final regression        │
│  → 更新计划证据                                                 │
...
- **trunk 不做环境副作用**：trunk 只负责 task context；任何 install、ignored local file sync、或私有配置生成都在 task worktree 内完成。
- **人工测试暂停节点**：`wt-dev` 在实现完成后输出测试清单并等待确认，自动化测试不替代人工验证。
- **人工测试后再次集成**：人工测试通过后仍需重新同步最新 trunk 并重跑最终回归，防止“验收后直接 merge”漏掉集成回归。
...
**约束**：一个 `task_id` 同时只允许一个活跃 workplan 和一个活跃 worktree。
...
| 4. Worktree Entry | Trunk → Worktree | `wt-plan` | Executor | 使用 Codex app handoff 或手动 `git worktree add` 进入该 task 的独立 worktree |
| 5. Environment Init | Worktree | `wt-dev` | Executor | 依赖安装、ignored local files / 私有配置准备、最小可运行检查（首次运行时执行） |
...
| 9. Final Regression + Plan Evidence | Worktree | `wt-dev` | Executor | 人工测试后再次同步 trunk，再跑全量回归；更新 `progress/findings`，保持任务为 `PLANNED` |
...
# 创建当前 task 的 workplan
plan_tracker.py quick-plan --task-id TC-007

# 恢复执行
plan_tracker.py quick-resume [--task-id <id>]
...
每个 task 在 `plans/workplans/<task_id>/` 下创建三个文件：

```text
plans/workplans/
└── TC-007/
    ├── task_plan.md               # 规划：目标、步骤
    ├── findings.md                # 调研：技术决策、风险、现有代码
    └── progress.md                # 进度：完成/进行中/阻塞/下一步
```

**恢复执行前必须先读取对应 task 目录下的三文件**，确保上下文不丢失。

### 信息职责分工

| 信息类型 | 记录位置 |
|----------|----------|
| 做了什么 / 做到哪了 | `plans/workplans/<task_id>/progress.md` |
| 为什么这样做 / 技术决策 | `plans/workplans/<task_id>/findings.md` |
| 已知风险 | `plans/workplans/<task_id>/findings.md` "Risks" section |
| 跨 task 依赖 | `todo_current.md` 的 `note` 字段 |
| Breaking change 记录 | `plans/workplans/<task_id>/findings.md` |
...
- 已 merge 回 trunk，且任务状态已更新为 `DONE`

### Completion Semantics

- `implementation complete`
  - 代码已实现，自动化与人工验证已通过，但尚未 merge / 标记 `DONE`
- `task complete`
  - 已 merge 回 trunk，且 `plans/todo_current.md` 状态为 `DONE`

只有 `task complete` 才算 WT-PM 闭环结束。
...
- 一个 `task_id` 同时只允许一个活跃 workplan 目录
- 恢复执行前必须先读取该 task 的三文件（防止上下文丢失）
...
| 项目 | 说明 |
|------|------|
| Plan 文件模板 | 可在 task 目录内增加如 `risks.md` |
