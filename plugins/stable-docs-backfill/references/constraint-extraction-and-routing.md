...
具体 destination 必须落在配置 `canonicalDocs` 的一个且仅一个 owner home。常见 role 可包含 `module-spec`、`module-prd`、`top-level-prd`、`architecture`、`context-language`、`tech-stack` 与 `hands-on`，但 Plugin 不写死仓库的 domain 名、目录或 module taxonomy。
