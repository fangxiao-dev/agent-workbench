...
4. 生成 3.5 candidate：删除 `tasks`、`resume`、DAG runtime projection；创建 `attemptHistory`、显式 `predecessors`、嵌套 `evidenceIndex` 和 `activeCheckpoints`。旧 Handoff 移入 `migration/archive/task-handoffs/`，不得进入 acceptance evidence。
