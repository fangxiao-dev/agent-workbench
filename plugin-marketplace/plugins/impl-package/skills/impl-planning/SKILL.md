...
- 必填 Attempt ID、阶段 A 兼容写法 `Composition: tickets=..., dag=false`、Coverage & Change Map、执行策略、Planned Verification、集成顺序和下一动作。
...
4. 选择 `tickets=true, dag=false` 或 `tickets=false, dag=false`，并说明 Ticket 是否 earned；`dag=true` 只允许在旧 package 迁移/恢复计划中出现。
...
- 若 Composition 为 `tickets=true`，earned Ticket 必须存在并与当前 Attempt 一致；`tickets=false` 的 Plan-direct 不创建 Ticket。旧 package 迁移计划如需读取 DAG，必须标记为 legacy-only；
