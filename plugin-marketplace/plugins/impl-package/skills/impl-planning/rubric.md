...
- `references/common-misjudgments.md` 只承载每条规则的具体误判后果（为什么错、错了会有什么下游代价），不重复 `SKILL.md` 正文已有的规则表述——2026-09-07 复核发现首版把误判条目压扁成规则复述，与正文重复，已改回保留原有“会导致 X 后果”的解释并删掉与正文重复的部分。
- 触发方式是逐步骤在 `SKILL.md` 对应句子后挂一句指针（对齐 req-align 的做法），不是开头一句覆盖 admission/拆票/审查/初始化的笼统前言——后者会让几乎每一步都要求先读 reference，等于没有真正减负。
