...
本 Skill 拥有 Ticket 的纵向切片、AC、contract references 和 typed dependency 语义；Ticket 文件的物理写入与运行时 state 更新由绑定的 `/impl-package:standing-bookkeeper` 执行。主 thread 不直接编辑当前 package 的 Ticket 或 `.impl-package/state.json`。
...
6. 新 package 使用 `tickets=true, dag=false` Ticket-only 合同；不创建 DAG，也不建立 Ticket/Task 双层 bundle。由 bookkeeper 发布当前 Attempt 的 Ticket，并通过现有 state CLI 原子推进为 Approved/PENDING。旧 package 的 `dag=true` 只读，不由本 skill 创建或更新。
