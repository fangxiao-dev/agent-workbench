...
6. 新 package 使用 `dag=false` 兼容占位；不创建 DAG，也不建立 Ticket/Task 双层 bundle。`init` 只发布当前 Attempt 的 Ticket，并将其原子推进为 Approved/PENDING。旧 package 的 `dag=true` 只读，不由本 skill 创建或更新。
...
Ticket 发布后不承载 Phase、Next、worker 或 implementation progress。语义变化使受影响 Ticket 回到 Draft/重验流程；纯 projection 修复可由 `refresh-progress` 完成。新合同使用 `RETIRED` 统一表示 waived/superseded，并要求记录对应 disposition；3.4 runtime 旧状态只作为迁移输入。
