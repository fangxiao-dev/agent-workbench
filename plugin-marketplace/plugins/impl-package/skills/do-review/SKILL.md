...
```text
python <do-review-skill-dir>/scripts/verify-reviewer-skills.py --plugin-root <impl-package-plugin-root>
python <do-review-skill-dir>/scripts/verify-reviewer-skills.py --plugin-root <impl-package-plugin-root> --skills <selected-skill-1> <selected-skill-2> <selected-skill-3>
python <do-review-skill-dir>/scripts/verify-reviewer-skills.py --plugin-root <impl-package-plugin-root> --skill-path <custom-skill>=<absolute-skill-path>
```

For an explicit reviewer selection, resolve each name once through the registry or active skill catalog, pass every resolved registry name with `--skills`, and pass every catalog-resolved custom name/path with `--skill-path`. A path must exist, be readable, remain inside the supplied plugin root, and have frontmatter whose `name` exactly matches the requested name. Fail before dispatch if any selection is ambiguous or invalid; never substitute a similarly named or removed skill.
...
Create the canonical ledger immediately after the base/head SHAs and review slug are fixed. Use `python <do-review-skill-dir>/scripts/review_ledger.py create` (or an equivalent temp-aware file operation) and record its absolute path in the shared context as `Canonical ledger artifact`. Resolve `<do-review-skill-dir>` from this skill's installed location; do not assume the target repository is the script directory. If the generated filename already exists, fail closed rather than overwriting it and start a new timestamped run. The main session must write the initial scope, source-discovery record, user classification policy, and round state before dispatch.
...
- Do not create, request, or infer owner approval. In a GO execution attempt, return findings to `/impl-package:dev-with-track` so it automatically repairs, verifies and evaluates the gate; a direct `/impl-package:do-review` invocation stops at the review checkpoint.
