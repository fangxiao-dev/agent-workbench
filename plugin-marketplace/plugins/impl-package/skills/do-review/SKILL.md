...
- **Aggregate fail-closed**: no required track → `INCOMPLETE`; any required `FAIL` makes Overall `FAIL`; otherwise any required `UNCERTAIN` makes Overall `UNCERTAIN`; otherwise any required track that is not `PASS` (missing verdict) → `INCOMPLETE`; PASS requires every required track. Terminal Safety omission follows the Safety admission above.
