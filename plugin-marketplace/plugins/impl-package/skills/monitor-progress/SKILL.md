...
- `packageStatus` 是正式 Ticket/Gate 状态的唯一运行期 authority；`targetUpdates` 只解释当前执行动作。两者矛盾时保留差异并以 package 为正式结论。
- 一个 observation topic 只承载一个能被未来消息独立修改的决策轴；一条消息改变多个轴时分别新增或更新，正式 Ticket 状态不写入 observation。
- 每条 observation 增加 `kind=specific|pattern`。先做实例替换测试：替换具体 Ticket、session、人员或本次时间条件后仍应约束后续同类场景时为 pattern，否则为 specific；混合消息拆开记录。pattern 使用稳定的条件、行为和边界，高置信时 confirmed、不确定时 candidate；specific 保留具体对象、动作与完成条件。
