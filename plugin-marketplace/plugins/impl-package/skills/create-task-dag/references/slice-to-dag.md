...
Tickets 与 DAG 进入 `ready-for-review` 前，必须联合验证 earned-ticket coverage、typed blockers 与 Task dependencies 的一致性、primary ownership 不重叠、`contributes-to` 多对多映射、AC evidence 可行性，以及同一 Attempt、gate/preflight 边界和 binding。DAG 不复制 Ticket AC、worker ownership 或 Task→AC 验收映射；Task `DONE` 也不表示 Ticket accepted。
