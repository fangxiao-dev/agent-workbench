...
如果 active skill catalog 中存在 `codebase-design`，优先读取其 deep module vocabulary；否则直接使用本段内置基线：检查 module interface 是否以小表面承载足够行为（depth / leverage）；检查知识与验证是否留在合适 seam 以维持 locality；检查新增 adapter 是否有真实可变性依据。interface 包含调用方必须知道的 invariant、错误模式和顺序约束，不得缩窄为类型签名。这些判断仍是 judgement baseline，不伪装成仓库未声明的硬规则。可选 skill 缺失不阻塞 Standards review。
