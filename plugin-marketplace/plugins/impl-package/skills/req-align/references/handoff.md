...
Do not infer downstream closure pending merely because it was not checked. Artifact Status and Gate Result must agree. After writes, include topic slug, package ID/path, current D/S aliases, gate results and evidence location, changed `execution-findings.md` only if appended, and remaining owner decisions. Do not require owners to read JSON. When Grill ran, name its temporary discussion record and summarize its outcomes.
