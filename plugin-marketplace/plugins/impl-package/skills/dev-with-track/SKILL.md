...
处境表漏掉某个当前处境时，按判断行动并走 escape 出口是合法路径，不是违规；若主控偏离渲染建议，也只需记录理由，不把 renderer 当成阻断器。**每一次 escape 必须写一行 `kind=escape` 轨迹，带 `subject`、`deviation` 和 `reason`**，包括偏离渲染建议和处境表没覆盖当前处境两种；escape 是唯一既不产生产物、也无法从其它行推导的事件，处境表只能靠这些行发现自己的缺陷。派发的发起与返回、finding 定级与分流、来源路由判断和需要声明的 fact 照常写入轨迹；Ticket 选择由随后 dispatch 的 `subject` 表明，不必单独写行。新增 `dispatch`、`escape`、`fact` 或 `worker-return` 事件时，主控调用 `python <impl-package-plugin-root>/scripts/impl_package_state.py --package <package> trail append`，把事件 payload 通过 stdin 交给 CLI；CLI 负责校验并追加。轨迹只追加、不改写已有行；发现某行写错时追加一条正确的 fact 或补一行说明，改写历史会让回放失去意义。显式交接使用 `recovery checkpoint --handoff`：先完成 state checkpoint，再按 trail schema 轮换活动文件，在新轨迹中保留仍成立的 fact 并记录 handoff；普通 checkpoint 不轮换。老 package 或异常补写仍可按维护文档手写轨迹。
