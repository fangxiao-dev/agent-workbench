...
4. **形成 Topic**：把当前业务动作切成 bounded outcome、ownership、禁改范围、comparison point、成功条件与局部验证；需要 mutation 的 PENDING Ticket 先完成对应 preflight。
...
- accepted finding 作为同 Topic work lane 的 bounded fix 交给 Dispatcher；下游 worker 遵循 SDD。review lane 独立于 work lane，是否复用 reviewer 由同 Topic lifecycle 决定。
