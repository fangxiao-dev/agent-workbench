...
| `fix` | finding 已确认且已边界化 | 答案为 `diff`；fix 不阻塞后续开发，「等这个 fix 修完」不是暂停主线的理由——同 Topic 复用原 lane 与原 worktree；与当前开发不同 Topic 且写入交叉较小时，默认在隔离 worktree 并行进行，修好后合入；不重新裁决 finding、不扩大范围、不宣称 closure |
