...
- 一次派发只对应一个 Dispatcher 已准入的 bounded outcome；investigate 只回答会改变 Topic 决策的子问题，不把“把整个方案/系统调研清楚”当作一个大目标。
- 多个知识来源共同回答同一个 Topic 决策时保持一次派发；是否拆分以 `$dispatcher` 的 Topic-first 门槛为准。
...
- 给 plan 里已经裁决、且由 Dispatcher 授权的 baby step；同一 implementer 可在主控消费 return 后，通过 follow-up 连续承接下一步，但不得自行跨越尚未授权的主控决策。不把整份 plan 都丢过去，也不把每个机械步骤重新派发。
