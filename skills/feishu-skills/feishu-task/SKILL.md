...
> - **走 [feishu-minutes](../feishu-minutes/SKILL.md) 的 `minutes +todo`**（禁止本 skill）：上下文含 **妙记 / 会议纪要 / minute_token / 妙记 URL**（`/minutes/`）；或「在某某妙记里新建/修改待办」「妙记 AI 待办」「会议录制里的待办」。
> - **走本 skill（feishu-task）**：任务清单、分配给我、项目待办、截止日期/提醒、子任务、任务清单成员；或 applink 含 `client/todo/task?guid=`；或明确说「飞书任务」「任务中心」「我的任务清单」。
...
| Shortcut | 说明 |
|----------|------|
| [`+create`](references/feishu-task-create.md) | create a task |
| [`+update`](references/feishu-task-update.md) | update task attributes |
| [`+set-ancestor`](references/feishu-task-set-ancestor.md) | set or clear a task ancestor |
| [`+comment`](references/feishu-task-comment.md) | add a comment to a task |
| [`+complete`](references/feishu-task-complete.md) | mark a task as complete |
| [`+reopen`](references/feishu-task-reopen.md) | reopen a completed task |
| [`+assign`](references/feishu-task-assign.md) | assign or remove task members |
| [`+followers`](references/feishu-task-followers.md) | manage task followers |
| [`+reminder`](references/feishu-task-reminder.md) | manage task reminders |
| [`+get-my-tasks`](references/feishu-task-get-my-tasks.md) | List tasks assigned to me |
| [`+get-related-tasks`](references/feishu-task-get-related-tasks.md) | list tasks related to me |
| [`+search`](references/feishu-task-search.md) | search tasks |
| [`+subscribe-event`](references/feishu-task-subscribe-event.md) | subscribe to task events |
| [`+upload-attachment`](references/feishu-task-upload-attachment.md) | upload a local file as an attachment to a task |
| [`+tasklist-create`](references/feishu-task-tasklist-create.md) | create a tasklist and optionally add tasks |
| [`+tasklist-search`](references/feishu-task-tasklist-search.md) | search tasklists |
| [`+tasklist-task-add`](references/feishu-task-tasklist-task-add.md) | add tasks to a tasklist |
| [`+tasklist-members`](references/feishu-task-tasklist-members.md) | manage tasklist members |
