...
| 想做什么 | user 身份 | bot 身份 |
|---|---|---|
| 按姓名 / 邮箱搜员工拿 open_id | [`+search-user`](references/feishu-contact-search-user.md) | 不支持 |
| 已知 open_id 取他人资料 | `+search-user --user-ids <id>` | [`+get-user --user-id <id>`](references/feishu-contact-get-user.md) |
| 查看自己 | `+get-user` 或 `+search-user --user-ids me` | 不支持 |
| 查同事的个人状态 / 签名 | `user_profiles batch_query` | 不支持 |

已知 open_id 只是想发消息 / 排日程,不必经过 contact —— 直接 [`feishu-im`](../feishu-im/SKILL.md) / [`feishu-calendar`](../feishu-calendar/SKILL.md)。
...
- **41050 / Permission denied** 受当前身份的可见范围限制(两条命令都可能遇到)。换 bot 身份或让管理员调整可见范围,细节见 [`feishu-shared`](../feishu-shared/SKILL.md)。
...
- 发消息 / 查聊天记录 → [`feishu-im`](../feishu-im/SKILL.md)
- 排日程 / 邀请会议 → [`feishu-calendar`](../feishu-calendar/SKILL.md)
- 部门树 / 按部门列员工 / 组织架构 → [`feishu-openapi-explorer`](../feishu-openapi-explorer/SKILL.md) 查找原生接口
