...
name: feishu-slides
version: 1.0.0
description: "飞书幻灯片：创建和编辑幻灯片。创建演示文稿、读取幻灯片内容、管理幻灯片页面（创建、删除、读取、局部替换）。当用户需要创建或编辑幻灯片、读取或修改单个页面时使用。当用户给出 doubao.com 的 /slides/ URL/token 时，也应直接使用本 skill，不要因为域名不是飞书而回退到 WebFetch；路由依据是 URL 路径模式和 token，而不是域名。不负责：云文档内容编辑（走 feishu-doc）、云文档里的独立画板对象（走 feishu-whiteboard，注意 slide 内嵌的流程图/架构图仍属本 skill）、上传或下载普通文件（走 feishu-drive）。"
metadata:
  requires:
    bins: ["lark-cli"]
...
| 大幅改写页面 | 先回读现有 XML，写入新 plan，再替换或重建相关页面 | `xml_presentations.get`、`+replace-slide`、`feishu-slides-edit-workflows.md` |
| 编辑单个标题、文本块、图片或局部元素 | 优先块级替换/插入，不改页序 | `slides +replace-slide`、`feishu-slides-replace-slide.md` |
...
| 在 slide 中绘制流程图、时序图、架构图、散点图、漏斗图或装饰图案 | 必须先用 Read 工具读取参考文档，再生成 `<whiteboard>` 元素 | [`feishu-slides-whiteboard.md`](references/feishu-slides-whiteboard.md) |
...
**CRITICAL — 开始前 MUST 先用 Read 工具读取 [`../feishu-shared/SKILL.md`](../feishu-shared/SKILL.md)，认证、权限和全局参数均以 feishu-shared 为准。**

**CRITICAL — 生成任何 XML 之前，MUST 先用 Read 工具读取 [xml-schema-quick-ref.md](references/xml-schema-quick-ref.md)，禁止凭记忆猜测 XML 结构。**

**CRITICAL — 新建演示文稿或大幅改写页面时，MUST 先生成 `.feishu-slides/plan/<deck-or-task-id>/slide_plan.json`，再生成 XML。先创建对应目录，规划层规则和中间产物生命周期见 [planning-layer.md](references/planning-layer.md)。仅替换一个标题、插入一个块等小型已有页编辑可豁免。**
...
**编辑已有幻灯片页面**：优先用 [`+replace-slide`](references/feishu-slides-replace-slide.md)（块级替换/插入，不动页序）；选择 action 和完整读-改-写流程见 [`feishu-slides-edit-workflows.md`](references/feishu-slides-edit-workflows.md)。
...
- 创建：[`feishu-slides-create.md`](references/feishu-slides-create.md)
- 编辑：[`feishu-slides-edit-workflows.md`](references/feishu-slides-edit-workflows.md)、[`feishu-slides-replace-slide.md`](references/feishu-slides-replace-slide.md)
- 图片：[`feishu-slides-media-upload.md`](references/feishu-slides-media-upload.md)
- 流程图 / 时序图 / 架构图 / 装饰图案：[`feishu-slides-whiteboard.md`](references/feishu-slides-whiteboard.md)
...
```bash
python3 skills/feishu-slides/scripts/template_tool.py search --query "<用户需求原文>" --limit 3
python3 skills/feishu-slides/scripts/template_tool.py summarize --template <template-id> --label <封面|目录|分节|内容|结尾>
python3 skills/feishu-slides/scripts/template_tool.py extract --template <template-id> --label <页型> --out /tmp/template-slice.xml
```
...
Step 2: 生成大纲 → 用户确认 → 写入 slide_plan.json
  ...
  - 新建 / 大幅改写必须先创建目录并写入 `.feishu-slides/plan/<deck-or-task-id>/slide_plan.json`
...
Step 3: 按 slide_plan.json 生成 XML → 创建
  ...
  - 创建方式按“创建方式选择”判断；图片、复杂 XML、转义和 3350001 排查按 feishu-slides-create.md、media-upload.md、troubleshooting.md 执行
...
| Shortcut | 说明 |
|----------|------|
| [`+create`](references/feishu-slides-create.md) | 创建 PPT（可选 `--slides` 一步添加页面，支持 `<img src="@./local.png">` 占位符自动上传） |
| [`+media-upload`](references/feishu-slides-media-upload.md) | 上传本地图片到指定演示文稿，返回 `file_token`（用作 `<img src="...">`），最大 20 MB |
| [`+replace-slide`](references/feishu-slides-replace-slide.md) | 对已有幻灯片页面进行块级替换/插入（`block_replace` / `block_insert`），自动注入 id 和 `<content/>`，不改变页序 |
...
1. **先规划再写 XML**：新建演示文稿或大幅改写页面时，必须先写入 `.feishu-slides/plan/<deck-or-task-id>/slide_plan.json`；模板、风格和大纲只能作为规划输入，不能绕过规划层
