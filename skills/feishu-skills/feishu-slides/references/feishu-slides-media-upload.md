...
详见 [+create 文档](feishu-slides-create.md#本地图片path-占位符)。

### 给已有 PPT 的已有页加图

拿到 `file_token` 后走 [`+replace-slide`](feishu-slides-replace-slide.md) 的 `block_insert`，不用搬原 XML、不改 `slide_id`、不打乱页序：
...
- [+create](feishu-slides-create.md) — 新建 PPT（支持 `@` 占位符自动上传图片）
- [+replace-slide](feishu-slides-replace-slide.md) — 给已有页加图 / 换图（`block_insert` / `block_replace`）
- [xml_presentation.slide create](feishu-slides-xml-presentation-slide-create.md) — 创建 slide 页面（拿到 file_token 后塞进 XML）
