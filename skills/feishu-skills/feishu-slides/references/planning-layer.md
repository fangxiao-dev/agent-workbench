...
新建演示文稿或大幅改写页面时，必须先写 `.feishu-slides/plan/<deck-or-task-id>/slide_plan.json`，再生成 XML。这个文件是 deck 的设计中间层，用来把叙事、页面角色、布局、视觉重点和文字密度固定下来，避免从用户提示直接跳到 XML。
...
3. 选择唯一 plan 目录：`.feishu-slides/plan/<deck-or-task-id>/`。
4. 先创建目录：`mkdir -p .feishu-slides/plan/<deck-or-task-id>`。
5. 写入 `.feishu-slides/plan/<deck-or-task-id>/slide_plan.json`。
...
模板不能代替 plan。模板搜索和摘要只能影响 `theme_style`、页面流、布局选择和局部布局骨架；最终仍必须有 `.feishu-slides/plan/<deck-or-task-id>/slide_plan.json`。
...
- Do not reuse `.feishu-slides/plan/slide_plan.json` as a shared path.
...
`.feishu-slides/` is local agent state. It supports recovery, iteration, and later edits, but it should not be treated as source code or committed by default.

Keep:

- `.feishu-slides/plan/<deck-or-task-id>/slide_plan.json` after successful creation or major rewrite. The plan is the editable design state for the deck.
