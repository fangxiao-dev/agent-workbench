...
编辑走 **shortcut [`+replace-slide`](feishu-slides-replace-slide.md)**（块级替换 / 插入），配合 `xml_presentation.slide.get` 读原页拿 `block_id`。
...
- [feishu-slides-replace-slide.md](feishu-slides-replace-slide.md) — +replace-slide shortcut 参数详情
- [feishu-slides-xml-presentation-slide-get.md](feishu-slides-xml-presentation-slide-get.md) — slide.get 参考（拿 `block_id` / `revision_id`）
- [feishu-slides-xml-presentation-slide-replace.md](feishu-slides-xml-presentation-slide-replace.md) — 底层 replace API 参考（一般直接用 shortcut 即可）
- [feishu-slides-media-upload.md](feishu-slides-media-upload.md) — 上传图片拿 file_token
