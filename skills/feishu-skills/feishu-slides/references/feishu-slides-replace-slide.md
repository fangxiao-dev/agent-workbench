...
> **前置条件：** 先阅读 [`../feishu-shared/SKILL.md`](../../feishu-shared/SKILL.md) 了解认证、全局参数和安全规则。
...
| `<img>` | 图片 | `src` 必须是 [`+media-upload`](feishu-slides-media-upload.md) 返回的 `file_token`，不能是 URL |
...
| `<whiteboard>` | 画板（SVG 或 Mermaid） | 内嵌 `<svg>` 或 `<mermaid>`；`slide.get` 返回结构不含内部数据，但可直接写完整新 XML 做 `block_replace` 覆盖；详见 [`feishu-slides-whiteboard.md`](feishu-slides-whiteboard.md) |
...
| `<img>` 不显示 / 显示破图 | `src` 写了外链 URL | 换成通过 [`+media-upload`](feishu-slides-media-upload.md) 拿到的 `file_token` |
...
- [xml_presentation.slide get](feishu-slides-xml-presentation-slide-get.md) — 读原页拿 `block_id` / `revision_id`
- [xml_presentation.slide replace](feishu-slides-xml-presentation-slide-replace.md) — 底层 replace API 参考
- [+media-upload](feishu-slides-media-upload.md) — 上传图片拿 `file_token`
