...
> The underlying `members.create` API is flagged `danger: true` in the schema browser, but adding a member is **not** confirmation-gated (no `--yes`). To revert, call [`+member-remove`](feishu-wiki-member-remove.md) with the same `(member_id, member_type, member_role)` tuple.
...
- The role switch (`admin` ⇄ `member`) is not a single update — call [`+member-remove`](feishu-wiki-member-remove.md) for the old role first, then `+member-add` with the new one.
