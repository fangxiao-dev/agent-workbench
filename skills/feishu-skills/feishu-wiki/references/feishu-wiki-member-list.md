...
- Use `member_id` from the output as `--member-id` for [`+member-remove`](feishu-wiki-member-remove.md); `member_type` and `member_role` must be passed exactly as listed to remove a grant.
