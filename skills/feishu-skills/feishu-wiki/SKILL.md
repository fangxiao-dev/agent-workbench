...
name: feishu-wiki
version: 1.0.1
description: "飞书知识库：管理知识空间、空间成员和文档节点。创建和查询知识空间、查看和管理空间成员、管理节点层级结构、在知识库中组织文档和快捷方式。当用户需要在知识库中查找或创建文档、浏览知识空间结构、查看或管理空间成员、移动或复制节点时使用。当用户给出 doubao.com 的 /wiki/ URL/token 时，也应直接使用本 skill，不要因为域名不是飞书而回退到 WebFetch；路由依据是 URL 路径模式和 token，而不是域名。不负责：上传文件到知识库节点下（走 feishu-drive）、编辑文档/表格/Base 内容（走 feishu-doc / feishu-sheets / feishu-base）。"
metadata:
  requires:
    bins: ["lark-cli"]
...
- 用户要**整理 / 盘点 / 归类 / 重构知识库、个人文档库、文档库目录或 Wiki 节点结构**，或要生成整理方案、目标目录树、移动计划时，不要只使用 Wiki 节点 API。必须先阅读 [`../feishu-drive/references/feishu-drive-workflow-knowledge-organize.md`](../feishu-drive/references/feishu-drive-workflow-knowledge-organize.md)，该 workflow 负责 Drive / Wiki / 个人文档库的统一入口解析、资源盘点、分类计划、写前确认和结果验证。
...
| Shortcut | 说明 |
|----------|------|
| [`+move`](references/feishu-wiki-move.md) | Move a wiki node, or move a Drive document into Wiki |
| [`+node-create`](references/feishu-wiki-node-create.md) | Create a wiki node with automatic space resolution |
| [`+delete-space`](references/feishu-wiki-delete-space.md) | Delete a wiki space, polling the async delete task when needed |
| [`+space-list`](references/feishu-wiki-space-list.md) | List all wiki spaces accessible to the caller |
| [`+space-create`](references/feishu-wiki-space-create.md) | Create a wiki space (user identity only) |
| [`+node-list`](references/feishu-wiki-node-list.md) | List wiki nodes in a space or under a parent node (supports pagination) |
| [`+node-copy`](references/feishu-wiki-node-copy.md) | Copy a wiki node to a target space or parent node |
| [`+node-get`](references/feishu-wiki-node-get.md) | Get a wiki node's details by node_token / obj_token / Lark URL |
| [`+node-delete`](references/feishu-wiki-node-delete.md) | Delete a wiki node, polling the async delete task when needed |
| [`+member-add`](references/feishu-wiki-member-add.md) | Add a member to a wiki space |
| [`+member-remove`](references/feishu-wiki-member-remove.md) | Remove a member from a wiki space |
| [`+member-list`](references/feishu-wiki-member-list.md) | List members of a wiki space (supports pagination) |
...
- 上传 / 下载文件到知识库节点下 → [`feishu-drive`](../feishu-drive/SKILL.md)（`drive +upload --wiki-token`）
- 编辑文档正文内容 → [`feishu-doc`](../feishu-doc/SKILL.md)
- 表格 / 多维表格数据操作 → [`feishu-sheets`](../feishu-sheets/SKILL.md) / [`feishu-base`](../feishu-base/SKILL.md)
- 按名称搜索文档 / Wiki / 表格文件、评论与权限管理 → [`feishu-drive`](../feishu-drive/SKILL.md)
