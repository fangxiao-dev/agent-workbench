...
version: 1.0.0
description: "飞书日历：管理日历日程和会议室。查看/搜索日程、创建/更新日程、管理参会人、查询忙闲和推荐时段、预定会议室。当用户需要查看日程安排、创建/修改会议、查询/预定会议室时使用。不负责：查询过去的视频会议记录（走 feishu-vc）、待办任务（走 feishu-task）。"
metadata:
  requires:
    bins: ["lark-cli"]
...
开始前先读 [`../feishu-shared/SKILL.md`](../feishu-shared/SKILL.md)（认证、权限处理）。

**CRITICAL — 凡涉及预约日程/会议或查询/搜索会议室，第一步 MUST 读 [`references/feishu-calendar-schedule-meeting.md`](references/feishu-calendar-schedule-meeting.md)。禁止跳过此步直接调用 API 或 Shortcut！**
...
| Shortcut | 说明 |
|----------|------|
| [`+agenda`](references/feishu-calendar-agenda.md) | 查看日程安排（默认今天） |
| [`+create`](references/feishu-calendar-create.md) | 创建日程并邀请参会人（ISO 8601 时间） |
| [`+update`](references/feishu-calendar-update.md) | 更新既有日程字段，或独立增量添加/移除参会人和会议室 |
| [`+freebusy`](references/feishu-calendar-freebusy.md) | 查询用户主日历的忙闲信息和 RSVP 状态 |
| [`+room-find`](references/feishu-calendar-room-find.md) | 针对一个或多个**明确的**时间块查找可用会议室（无明确时间时禁止直接调用，需先走 +suggestion） |
| [`+rsvp`](references/feishu-calendar-rsvp.md) | 回复日程（接受/拒绝/待定） |
| [`+suggestion`](references/feishu-calendar-suggestion.md) | 根据非明确时间或一段时间范围，推荐多个可用时间块方案 |

## 前置条件路由

| 场景 | 前置要求 |
|------|----------|
| 预约日程/会议、查会议室 | 先读 [feishu-calendar-schedule-meeting.md](references/feishu-calendar-schedule-meeting.md) |
...
| 用户意图 | 路由到 |
|----------|--------|
| 查询过去的会议（"昨天的会议""上周的会"） | [`../feishu-vc/SKILL.md`](../feishu-vc/SKILL.md)（会议数据含即时会议，仅查日程会遗漏） |
| 查询日历/日程或未来时间的会议 | 本 skill |
| 预约/改约日程、添加/移除参会人、添加/更换会议室、调整时间 | 先判断新建 vs 编辑，再进入 [schedule-meeting 工作流](references/feishu-calendar-schedule-meeting.md) |
...
- 凡是"预定/查询/搜索可用会议室"，都必须进入 [schedule-meeting 工作流](references/feishu-calendar-schedule-meeting.md)。
...
- 查询过去的视频会议记录 → [feishu-vc](../feishu-vc/SKILL.md)
- 待办任务管理 → [feishu-task](../feishu-task/SKILL.md)
- 会议室物理设施管理 → 管理员后台
