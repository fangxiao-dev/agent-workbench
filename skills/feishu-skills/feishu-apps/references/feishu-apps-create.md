...
用来创建应用资产并拿到 `app_id`。它不负责把自然语言需求交给云端 Agent：用户要“帮我生成/迭代应用”时，先创建 `full_stack` app，再进入 [`feishu-apps-cloud-dev.md`](feishu-apps-cloud-dev.md) 用 `+session-create` / `+chat` 提交需求。
...
- 发布现成 HTML/静态目录：读 [`feishu-apps-html-publish.md`](feishu-apps-html-publish.md)。
- 本地全栈开发：读 [`feishu-apps-local-dev.md`](feishu-apps-local-dev.md)。
- 云端 Agent 生成/迭代：读 [`feishu-apps-cloud-dev.md`](feishu-apps-cloud-dev.md)。
