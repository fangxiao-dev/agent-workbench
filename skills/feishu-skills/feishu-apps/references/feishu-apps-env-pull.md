...
> **前置条件：** 先阅读 [`../feishu-shared/SKILL.md`](../../feishu-shared/SKILL.md)（认证 / 全局参数 / 安全）。
...
`missing_scope`（没拿到 `spark:app:read`）时，按 feishu-shared 引导 `lark-cli auth login --domain apps`。其余失败优先转述 `error.hint` / `error.message`。

## 参考

- [feishu-apps](../SKILL.md) — 妙搭应用全部命令 + 心智模型
- [feishu-apps-local-dev](feishu-apps-local-dev.md) — 本地全栈开发端到端流程
