...
> **前置条件：** 先阅读 [`../../feishu-shared/SKILL.md`](../../feishu-shared/SKILL.md) 了解通用安全规则。
...
写信链路（`+send` / `+draft-create` / `+reply` / `+reply-all` / `+forward` / `+draft-edit` body op）已**强制内置**同一份 lint，提交前会自动净化 HTML。默认 envelope 不携带任何 lint 字段以保持响应小巧；加 `--show-lint-details` 可拿到完整 `lint_applied[]` / `original_blocked[]` 两个 Finding 数组（不再返回任何 `*_count` 字段，调用方需要 count 时 `len(arr)` 即可，详见 [邮件 HTML 写法指南](./feishu-mail-html.md#写信-shortcut-的-lint-返回值)）。本命令是写信链路 lint 的预览版，行为一致，调用更轻量，适合：
...
- 写信 shortcut（已内置同一份 lint）：[`+send`](./feishu-mail-send.md) / [`+draft-create`](./feishu-mail-draft-create.md) / [`+reply`](./feishu-mail-reply.md) / [`+reply-all`](./feishu-mail-reply-all.md) / [`+forward`](./feishu-mail-forward.md) / [`+draft-edit`](./feishu-mail-draft-edit.md)
- 知识文档：[邮件 HTML 写法指南](./feishu-mail-html.md)
