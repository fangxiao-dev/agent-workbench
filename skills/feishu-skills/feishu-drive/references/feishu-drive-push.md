...
- [feishu-drive](../SKILL.md) —— 云空间（云盘/云存储）全部命令
- [feishu-shared](../../feishu-shared/SKILL.md) —— 认证和全局参数
- [feishu-drive-status](feishu-drive-status.md) —— 上传前先看差异（避免全量回写）
- [feishu-drive-pull](feishu-drive-pull.md) —— Drive → 本地的对称命令
- [feishu-drive-upload](feishu-drive-upload.md) —— 单文件按需上传
