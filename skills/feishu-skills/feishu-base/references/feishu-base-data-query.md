...
> **入口指南**: [feishu-base-data-query-guide.md](feishu-base-data-query-guide.md) | **前置条件**: 先阅读 [`../feishu-shared/SKILL.md`](../../feishu-shared/SKILL.md) 了解认证、全局参数和安全规则。

本文档是 `+data-query` JSON DSL 的单一事实来源（SSOT），用于说明完整字段、操作符、限制、返回和错误恢复。常用 fewshot 与命令选择先读 [feishu-base-data-query-guide.md](feishu-base-data-query-guide.md)。

查询类任务还必须先遵守 [`feishu-base-data-analysis-sop.md`](feishu-base-data-analysis-sop.md)。`+data-query` 适合让筛选、分组、聚合、排序和 TopN 在 Base 云端查询服务中执行；不要用默认分页的 `+record-list` 或本地 `jq` 替代聚合查询。
...
- [feishu-base](../SKILL.md) — 多维表格全部命令
- [feishu-shared](../../feishu-shared/SKILL.md) — 认证和全局参数
- [feishu-base-data-analysis-sop.md](feishu-base-data-analysis-sop.md) — 查询范围、选路、下推、分页、`+record-list` / `+record-search` 回查和关系查询 SOP
- [feishu-base-cell-value.md](feishu-base-cell-value.md) — CellValue 格式规范
- [feishu-base-field-json.md](feishu-base-field-json.md) — 字段类型与 JSON 结构
