...
### 场景 2：加入会议 → 会后进入 feishu-vc 获取会议产物信息
...
后续按 `feishu-vc` 的产物决策处理：根据 `note_display_type`、`note_id`、`minute_token` 和用户意图选择纪要正文、逐字稿或妙记。
...
- [feishu-vc-agent-meeting-leave](feishu-vc-agent-meeting-leave.md) — 对应的离会命令
- [feishu-vc-agent-meeting-list-active](feishu-vc-agent-meeting-list-active.md) — 发现当前可读事件的进行中会议 ID
- [feishu-vc-agent-meeting-events](feishu-vc-agent-meeting-events.md) — 会中事件流
- [feishu-vc-search](../../feishu-vc/references/feishu-vc-search.md) — 搜索历史会议记录
- [feishu-vc-recording](../../feishu-vc/references/feishu-vc-recording.md) — 查询 minute_token
- [feishu-vc-notes](../../feishu-vc/references/feishu-vc-notes.md) — 获取会议纪要
- [feishu-vc-agent](../SKILL.md) — Agent 参会能力（本 skill）
- [feishu-vc](../../feishu-vc/SKILL.md) — 视频会议原子域（Meeting / Note 等核心概念）
