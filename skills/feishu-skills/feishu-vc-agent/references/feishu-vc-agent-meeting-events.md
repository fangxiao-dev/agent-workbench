...
- 如果会议已经结束，不要卡在 `+meeting-events`：  
  ...
  - 再根据 `note_display_type`、`note_id`、`minute_token` 和用户意图，按 `feishu-vc` 的产物决策读取纪要正文、逐字稿或妙记。
...
- [feishu-vc-agent-meeting-join](feishu-vc-agent-meeting-join.md) — 先真实入会
- [feishu-vc-agent-meeting-list-active](feishu-vc-agent-meeting-list-active.md) — 发现当前可读事件的进行中会议 ID
- [feishu-vc-agent-meeting-leave](feishu-vc-agent-meeting-leave.md) — 用户明确要求时离会
