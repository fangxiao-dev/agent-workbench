...
如果用户只是要求会议结束后拉录制、纪要或逐字稿，不要先调用 `+meeting-leave`；直接跨到 `feishu-vc` 查询会后产物。

```bash
# 第 1 步：会议结束后进入 feishu-vc 获取会议产物信息
...
- [feishu-vc-agent-meeting-join](feishu-vc-agent-meeting-join.md) — 对应的入会命令
...
- [feishu-vc-search](../../feishu-vc/references/feishu-vc-search.md) — 搜索历史会议（获取 meeting_id）
