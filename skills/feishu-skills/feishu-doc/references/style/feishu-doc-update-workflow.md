...
1. **选择读取范围**（节省上下文的关键）：
   ...
   - 详见 [`feishu-doc-fetch.md`](../feishu-doc-fetch.md) "意图引导：选择正确的 --scope"
2. 系统性评估：用户想改什么、现有文档风格是什么、哪些内容需要保留、哪些问题影响理解
3. **画板意图识别**：逐章节扫描，按 `feishu-doc-style.md`「画板意图识别」表判断哪些段落的信息适合用图表达。重要信息优先画板化，记录需要插图的章节（block ID）、推荐画板类型、mermaid/SVG路径和源内容片段
