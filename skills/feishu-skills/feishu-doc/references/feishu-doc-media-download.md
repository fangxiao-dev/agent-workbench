...
- 用户只是想查看、预览图片或文件素材时，优先使用 [`docs +media-preview`](feishu-doc-media-preview.md)
...
- 若你是从文档内容里提取：`feishu-doc-fetch` 返回的 Markdown 里可能包含：
  - 图片：`<image token="..." .../>`
  - 文件：`<file token="..." name="..."/>`
  - 画板：`<whiteboard token="..."/>`
...
- 如果报错返回的信息包含 `HTTP 403`，且目标是图片/文件素材，可以改成调用 [`docs +media-preview`](feishu-doc-media-preview.md) 看是否能先预览内容

## 参考

- [feishu-doc-fetch](feishu-doc-fetch.md) — 获取文档内容（用于提取 token）
