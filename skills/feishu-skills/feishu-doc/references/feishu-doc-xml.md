...
- `<whiteboard>` — 简单图由 SubAgent 直接插入 `<whiteboard type="svg">完整自包含 SVG</whiteboard>`；复杂图使用 `<whiteboard type="blank"></whiteboard>` 先创建空白画板，再按 [`feishu-doc-whiteboard.md`](feishu-doc-whiteboard.md) 启动 SubAgent 调用 `feishu-whiteboard` 写入；
...
> 详见 [feishu-doc-update.md](feishu-doc-update.md)。
