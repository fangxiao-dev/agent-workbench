...
> 1. [`feishu-doc-xml.md`](feishu-doc-xml.md) — XML 语法规则（使用 Markdown 格式时改读 [`feishu-doc-md.md`](feishu-doc-md.md)）
> 2. [`feishu-doc-update-workflow.md`](style/feishu-doc-update-workflow.md) — 改写增强工作流（Code-Act Loop、并行执行策略）
>
> **需要使用 callout、grid、table、whiteboard 等富 block，或用户明确要求美化/重排版时，再参考 [`feishu-doc-style.md`](style/feishu-doc-style.md)。该文件是表达组件参考，不是固定模板。**
>
> **未读完以上文件就生成内容会导致格式错误。**
...
| `warnings` | 警告信息列表 |
| `document.new_blocks` | 本次操作新增的 block 列表（如画板）。`block_id` 可用于后续精确编辑；`block_token` 是资源块 token（如画板）可交给 `feishu-whiteboard` 等 skill 继续操作 |
...
> **`docs +update` 不能直接编辑已有画板的内容。** 本命令只能**新增**画板块；要修改已有画板，先用 `docs +fetch --api-version v2` 取到 `<whiteboard token="...">`，再按 [`feishu-doc-whiteboard.md`](feishu-doc-whiteboard.md) 启动 SubAgent 读取 [`feishu-whiteboard`](../../feishu-whiteboard/SKILL.md) 并写入。

画板的语法选型与插入示例见 [`feishu-doc-style.md`](style/feishu-doc-style.md) 的「画板语法与插入」章节。
...
- **表达形式**：插入或替换内容时，优先沿用用户要求和已有文档风格；需要结构化表达时可参考 [`feishu-doc-style.md`](style/feishu-doc-style.md)，但不要为了固定丰富度主动添加组件

## 参考

- [`feishu-doc-update-workflow.md`](style/feishu-doc-update-workflow.md) — 改写增强工作流（Code-Act Loop、并行执行策略）
- [`feishu-doc-style.md`](style/feishu-doc-style.md) — 文档样式指南（元素选择 + 丰富度规则 + 颜色语义）
- [`feishu-doc-xml.md`](feishu-doc-xml.md) — XML 语法规范
- [`feishu-doc-fetch.md`](feishu-doc-fetch.md) — 获取文档
- [`feishu-doc-create.md`](feishu-doc-create.md) — 创建文档
- [`feishu-doc-media-insert.md`](feishu-doc-media-insert.md) — 插入图片/文件到文档
