...
> 2. [`feishu-doc-create-workflow.md`](style/feishu-doc-create-workflow.md) — 从零创作工作流（Code-Act Loop、并行执行策略）
>
...
- **`document.new_blocks`**：本次操作新增的 block 列表（如画板）。`block_id` 可用于 `docs +update` 的 `--block-id` 做精确编辑；`block_token` 是资源块（如画板）的 token，可交给 `feishu-whiteboard` 等 skill 继续操作
...
- **表达形式**：由用户目标和内容决定。需要结构化表达时可参考 [`feishu-doc-style.md`](style/feishu-doc-style.md)，但不要默认套用固定开头、固定富 block 比例或固定图表

## 参考

- [`feishu-doc-create-workflow.md`](style/feishu-doc-create-workflow.md) — 从零创作工作流（Code-Act Loop、并行执行策略）
...
- [`feishu-doc-update.md`](feishu-doc-update.md) — 更新文档
