...
|---|---|---|
| [`+feed-group-list`](feishu-im-feed-group-list.md) | List your feed groups | Its `--page-all` correctly merges the live and soft-deleted lists. No enrichment |
| [`+feed-group-list-item`](feishu-im-feed-group-list-item.md) | List the feed cards inside a group | Enriches each card with `chat_name` |
| [`+feed-group-query-item`](feishu-im-feed-group-query-item.md) | Look up feed cards in a group by ID | Enriches each card with `chat_name` |
...
Shortcut-only: [`+feed-group-query-item`](feishu-im-feed-group-query-item.md). Looks up feed cards in a group by an explicit ID list and enriches each with `chat_name`. There is no raw command — flags and response shape are in the linked shortcut doc.

## list_item

Shortcut-only: [`+feed-group-list-item`](feishu-im-feed-group-list-item.md). Lists the feed cards inside a group (paginated, `--page-all` supported) and enriches each with `chat_name`. There is no raw command — flags and response shape are in the linked shortcut doc.
...
- [`+feed-group-list`](feishu-im-feed-group-list.md) — `im:feed_group_v1:read`
- [`+feed-group-list-item`](feishu-im-feed-group-list-item.md) / [`+feed-group-query-item`](feishu-im-feed-group-query-item.md) — `im:feed_group_v1:read` **plus** `im:chat:read` (they always resolve `chat_name`)
