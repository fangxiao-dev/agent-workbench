...
| `note_display_type=unified` + 用户要逐字稿 / 原始记录 | `note +transcript --note-id <note_id>` → 切到 [feishu-note](../../feishu-note/SKILL.md) |
...
| `artifacts.todos` | 待办事项（JSON 内联，**只读**）；每条含 `content`、`is_done` 及 `todo_id`。`todo_id` 仅供 [`minutes +todo`](../../feishu-minutes/references/feishu-minutes-todo.md) 更新/删除待办时使用，不必展示给用户。**新建**妙记内待办请用 `minutes +todo`，不要用 feishu-task |
...
- [feishu-vc](../SKILL.md) — 视频会议全部命令
- [feishu-vc-search](feishu-vc-search.md) — 搜索历史会议（获取 meeting_id）
