...
- **feishu-calendar skill** 负责日历与日程管理，包括创建、查询、修改、删除日程等操作。
...
> **路由规则**：查询过去已结束的会议 → `feishu-vc`；查询未来日程/待开的会 → `feishu-calendar`；查询"今天有哪些会议" → 两者结合（`vc +search` 查已结束 + `calendar` 查未开始）。

## VC 域

- **feishu-vc skill** 负责视频会议管理，包括搜索历史会议、查询会议产物（智能纪要、逐字稿、妙记等）、查询参会人快照等操作。
...
详细用法请阅读 [`feishu-vc-search.md`](feishu-vc-search.md)。
...
详细用法请阅读 [`feishu-vc-notes.md`](feishu-vc-notes.md)。
...
可获取妙记的总结、待办、章节、文字记录等信息。详细用法请阅读 [`feishu-vc-notes.md`](feishu-vc-notes.md)。
...
详细用法请参考 [feishu-doc](../../feishu-doc/SKILL.md) 与 [feishu-note](../../feishu-note/SKILL.md) skill。
...
- 已知 `note_id` 后切到 [feishu-note](../../feishu-note/SKILL.md)；逐字稿路由以 `feishu-note` 的 `note_display_type` 规则为准。
...
- **feishu-doc skill** 负责飞书云文档管理，包括获取文档元信息、读取文档内容、创建和编辑文档等操作。
- **会议产物的文档本质**：智能纪要（`note_doc_token`）和 `normal` 纪要的逐字稿（`verbatim_doc_token`）都是飞书文档，需要通过 `feishu-doc` 的 API（如 `docs +fetch --api-version v2`）查询其内容和元信息；`unified` 纪要的逐字稿不是独立文档，用 `note +transcript` 拉取（[feishu-note](../../feishu-note/SKILL.md)）。
