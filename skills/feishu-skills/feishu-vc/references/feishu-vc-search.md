...
`vc +search` 只能搜索已结束的历史会议记录，不用于查询未来日程。查询未来会议安排请使用 [feishu-calendar](../../feishu-calendar/SKILL.md)。
...
| 搜不到未来会议 | `vc +search` 只查历史会议 | 改用 [feishu-calendar](../../feishu-calendar/SKILL.md) 查询未来日程 |
...
- [feishu-vc-recording](feishu-vc-recording.md) -- 查询会议对应的 minute_token
- [feishu-vc-notes](feishu-vc-notes.md) -- 获取会议纪要
