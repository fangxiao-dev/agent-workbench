...
name: feishu-vc
version: 1.0.0
description: "飞书视频会议：搜索历史会议记录、查询会议纪要（总结/待办/章节/逐字稿）、查询参会人快照。当用户查询已结束的会议、获取会议产物（纪要/妙记）、查看参会人时使用；查询未来日程走 feishu-calendar。不负责：Agent 真实入会/离会、会中实时事件（走 feishu-vc-agent）。"
metadata:
  requires:
    bins: ["lark-cli"]
...
| Shortcut | 说明 |
|----------|------|
| [`+search`](references/feishu-vc-search.md) | 搜索历史会议记录（需至少一个筛选条件） |
| [`+notes`](references/feishu-vc-notes.md) | 查询会议纪要和妙记产物（通过 meeting-ids、minute-tokens 或 calendar-event-ids） |
| [`+recording`](references/feishu-vc-recording.md) | 通过 meeting-ids 或 calendar-event-ids 查询 minute_token |
...
| 查日历/日程或未来时间的会议 | [feishu-calendar](../feishu-calendar/SKILL.md) |
| 查"今天有哪些会议" | `vc +search`（已结束）+ feishu-calendar（未开始），合并展示 |
| 只按自然语言标题查"xx 纪要的逐字稿 / 原始记录 / 谁说了什么" | 先到 [feishu-drive](../feishu-drive/SKILL.md) / [feishu-doc](../feishu-doc/SKILL.md)；仅在已拿到 `note_id` / `vc-node-id` 后再到 [feishu-note](../feishu-note/SKILL.md) |
| Agent 真实入会/离会、会中实时事件 | [feishu-vc-agent](../feishu-vc-agent/SKILL.md) |
| 本地音视频文件转纪要/逐字稿 | 先走 [feishu-minutes](../feishu-minutes/SKILL.md) 上传，再回 `vc +notes --minute-tokens` |

## 核心概念

- **视频会议（Meeting）**：飞书视频会议实例，通过 meeting_id 标识。已结束的会议支持通过关键词、时间段、参会人、组织者、会议室等条件搜索（见 `+search`）。
- **会议纪要（Note）**：视频会议结束后生成的结构化文档，通过 `note_id` 标识，包含纪要文档（总结、待办）和逐字稿文档。`note_display_type` 区分**普通纪要（`normal`）**和 **unified 纪要**；已知 `note_id` 的直查与 unified 原始记录请用 [feishu-note](../feishu-note/SKILL.md)。
...
> **逐字稿路由**：先看 `vc +notes` 返回的 `note_display_type`，不要只看 `verbatim_doc_token` 是否为空。具体路由以 [`+notes`](references/feishu-vc-notes.md) 和 [feishu-note](../feishu-note/SKILL.md) 为准。
>
...
### 1. 搜索会议记录
1. 仅支持搜索已结束的会议，对于还未开始的未来会议，需要使用 feishu-calendar 技能。
...
> - 用户说"逐字稿""完整记录""谁说了什么"时 → 按 `note_display_type` 路由，详见 [`+notes`](references/feishu-vc-notes.md)
> - 用户说"纪要""总结""纪要内容"时，应同时返回 `note_doc_token` 和 `meeting_notes`（如有）
> - 用户意图不明确时，应展示所有文档链接让用户选择，而不是替用户决定
> - 如果用户提供的是**本地音视频文件**并说"转纪要""转逐字稿"，不要直接从 `vc +notes` 开始；应先用 [minutes +upload](../feishu-minutes/references/feishu-minutes-upload.md) 生成 `minute_url`，再提取 `minute_token` 调用 `vc +notes --minute-tokens`
...
| 已结束会议的发言内容 | 先 `vc +notes`，再按 `note_display_type` 路由 | 本 skill / [`feishu-note`](../feishu-note/SKILL.md) |
| **进行中会议**的实时事件流（转写、聊天、共享、会中加入/离开）| `vc +meeting-events` | [`feishu-vc-agent`](../feishu-vc-agent/SKILL.md) |
| **Agent 真实入会 / 离会** | `vc +meeting-join` / `vc +meeting-leave` | [`feishu-vc-agent`](../feishu-vc-agent/SKILL.md) |
...
│   ├── UnifiedTranscript (unified 原始记录) ← unified 路径，note +transcript（feishu-note）
│   └── SharedDoc (会中共享文档)
...
> **妙记边界**：`+notes` 负责纪要内容、逐字稿和 AI 产物；妙记基础信息请优先看 [`+recording`](references/feishu-vc-recording.md) 与 [feishu-minutes](../feishu-minutes/SKILL.md)。
>
> **Note 域边界**：`vc +notes` 是从**会议线索**（`meeting_id` / `calendar_event_id` / `minute_token`）定位纪要的入口，返回 `note_id` 和 `note_display_type`。
> - 已有 `note_id` → [feishu-note](../feishu-note/SKILL.md)。
> - 已有 `doc_token` 且目标是读正文 → [feishu-doc](../feishu-doc/SKILL.md)。
> - 只有自然语言纪要标题 → 文档搜索 / Docx 正文读取；有显式 `vc-node-id` 才进入 [feishu-note](../feishu-note/SKILL.md)。
...
### minutes（跨域，详见 [feishu-minutes](../feishu-minutes/SKILL.md)）
...
- Agent 真实入会/离会、会中实时事件 → [feishu-vc-agent](../feishu-vc-agent/SKILL.md)
- 只有纪要文档标题的逐字稿查询 → 文档搜索 / Docx 正文读取；有显式 `vc-node-id` 才进入 [feishu-note](../feishu-note/SKILL.md)
- 本地音视频文件转纪要/逐字稿 → [feishu-minutes](../feishu-minutes/SKILL.md)（上传后回 `vc +notes`）
- 妙记搜索/下载/上传/重命名/替换说话人 → [feishu-minutes](../feishu-minutes/SKILL.md)
