...
身份：仅使用 `--as user`。使用前阅读 [`../feishu-shared/SKILL.md`](../feishu-shared/SKILL.md)。
...
- `note_doc_token`：纪要正文文档，正文读取交给 [feishu-doc](../feishu-doc/SKILL.md)。
- `verbatim_doc_token`：普通纪要逐字稿文档；unified 逐字稿不按这个 token 路由。

## 不在本 Skill 范围

- 通过 `meeting_id` / `calendar_event_id` / `minute_token` 定位纪要 → [feishu-vc](../feishu-vc/SKILL.md)。
- 自然语言纪要标题搜索 → [feishu-drive](../feishu-drive/SKILL.md) / [feishu-doc](../feishu-doc/SKILL.md)。
- Docx 正文读取 → [feishu-doc](../feishu-doc/SKILL.md)。
- 妙记基础信息与媒体文件 → [feishu-minutes](../feishu-minutes/SKILL.md)。

## Shortcuts

| Shortcut | 何时读 reference |
|----------|------|
| [`+detail`](references/feishu-note-detail.md) | 需要解释输出字段或根据展示类型继续路由 |
| [`+transcript`](references/feishu-note-transcript.md) | 需要拉取 unified 原始记录或处理本地输出文件 |
