...
如果以上命令无法覆盖需求（CLI 没有对应的已注册 API 或 Shortcut），使用 [lark-intl-openapi-explorer](../lark-intl-openapi-explorer/SKILL.md) 从 Lark 官方文档库逐层挖掘原生 OpenAPI 接口，获取完整的方法、路径、参数和权限信息，再通过 `lark-cli api` 裸调完成任务。
...
> **前置条件：** 先阅读 [`../lark-intl-shared/SKILL.md`](../lark-intl-shared/SKILL.md)。
