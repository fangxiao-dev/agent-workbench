---
name: lark-intl-minutes
version: 1.0.0
description: "Lark妙记：获取妙记基础信息（标题、封面、时长）和相关的 AI 产物（总结、待办、章节），下载妙记音视频文件。Lark妙记的 URL 格式为: http(s)://<host>/minutes/<minute-token>"
metadata:
  requires:
    bins: ["lark-cli"]
...
1. **提取 Token**：
   ...
   - 示例：从 `https://domain.larksuite.com/minutes/obc123456?project=xxx` 中提取出 `obc123456`。
