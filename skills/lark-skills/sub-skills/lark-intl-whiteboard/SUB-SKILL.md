---
name: lark-intl-whiteboard
description: >
  当用户要求在Lark云文档中绘制图表，或使用Lark画板绘制架构图、流程图、思维导图、时序图或其他可视化图表时使用此 skill。
...
Step 3: 渲染 & 审查 → 交付
  ...
  - 没问题 → 交付：
    · 用户要求上传Lark → 见下方”上传Lark画板”章节中的说明
    · 用户未指定 → 展示 PNG 图片给用户
```
...
> 上传需要Lark认证。遇到认证或权限错误时，阅读 [`../lark-intl-shared/SUB-SKILL.md`](../lark-intl-shared/SUB-SKILL.md) 了解登录和权限处理。
...
关于Lark文档的创建，读取等更多操作，请参考 lark-intl-doc skill [`../lark-intl-doc/SUB-SKILL.md`](../lark-intl-doc/SUB-SKILL.md)。
