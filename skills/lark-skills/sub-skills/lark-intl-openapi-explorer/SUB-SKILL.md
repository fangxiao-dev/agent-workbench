---
name: lark-intl-openapi-explorer
version: 1.0.0
description: "Lark 原生 OpenAPI 探索：从官方文档库中挖掘未经 CLI 封装的原生 OpenAPI 接口。当用户的需求无法被现有 lark-intl-* skill 或 lark-cli 已注册命令满足，需要查找并调用原生 Lark OpenAPI 时使用。"
metadata:
  requires:
    bins: ["lark-cli"]
---
...
> **前置条件：** 先阅读 [`../lark-intl-shared/SUB-SKILL.md`](../lark-intl-shared/SUB-SKILL.md) 了解认证、身份切换和安全规则。

当用户的需求**无法被现有 skill 或 CLI 已注册 API 覆盖**时，使用本技能从Lark官方 markdown 文档库中逐层挖掘原生 OpenAPI 接口，然后通过 `lark-cli api` 裸调完成任务。

## 文档库结构

Lark OpenAPI 文档以 markdown 层级组织：
...
- Lark International 使用 `open.larksuite.com`
...
- [lark-intl-shared](../lark-intl-shared/SUB-SKILL.md) — 认证和全局参数
- [lark-intl-skill-maker](../lark-intl-skill-maker/SUB-SKILL.md) — 如需将挖掘到的 API 固化为新 Skill
