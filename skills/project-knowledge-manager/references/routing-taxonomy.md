...
| `docs/module-knowledge/` | Module intent and behavior contracts. Route module-layer deltas to `backfill-stable-docs`; audit owns read-only inventory, apply owns approved item application, and verify owns independent validation. |
...
4. Hand verified module behavior to `backfill-stable-docs`; audit proposes candidates, apply performs only owner-approved compaction, and verify independently checks the result.
5. Route only reusable implementation or debugging lessons with reverse-lookup value into `docs/hands-on-knowledge/`.

Treat `req` files as inbox and audit records, not final long-term PRDs. Module-layer classification belongs to audit; first-PRD gates and approved pending handling belong to apply; validation belongs to verify. This taxonomy only routes work to the `backfill-stable-docs` maintainer.
