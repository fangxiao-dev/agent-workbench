...
In this skill, `backfill-stable-docs`, `audit-stable-docs`, `apply-stable-docs`, and `verify-stable-docs` are logical role or stage names. In Codex, invoke the Plugin through `$stable-docs-backfill:backfill-stable-docs`; the thin router selects a stage from user intent and does not execute audit, apply, or verify itself.
