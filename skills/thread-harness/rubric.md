...
- Owner 明确 child registry 的 `active` 缺失按 `true` 处理；poll、children 上限、worktree/branch 唯一性只作用于 active child；inactive 保留。~~done 不自动退休但不进入 `idle_nodes`~~ —— **R5 已改为显式 reassignment signal，见下**。
- Owner 明确 controller 是 ledger 唯一写入者；child 发送包含 `v/event/state/head/waiting_on/artifact/details/note` 的结构化 H1；controller 从消息来源绑定 current session，并验证 registry 与 worktree HEAD 关系后写 report/seam/decide。
