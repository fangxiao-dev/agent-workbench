...
- Owner 明确 replacement source child 只在 `%TEMP%\\codex-thread-harness\\handoffs` 写紧凑 prepare-only card，不写入 worktree、不提交；发送 path + SHA-256 + H1，由 controller 验证后 create/register/验收；恢复 authority 不变。
