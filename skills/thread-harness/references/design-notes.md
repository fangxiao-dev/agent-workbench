...
**使命与方法完全不变**：`/impl-package:impl-package` 主流程，执行阶段由 `/impl-package:dev-with-track` 提供业务事实并消费 `$dispatcher` 的执行协作结果。
...
`$handoff-to-new-session` 统一提供两阶段流程：第一阶段 child 只核对锚点然后停住，第二阶段才真正开工。Harness 只利用这个通用停顿窗口更新 registry；不再拥有或复制 anchor prompt、命名和创建配置。
...
**为什么不另造中间态 handoff 卡片**：通用 skill 已经拥有 anchor prompt 与 continuation prompt；再写临时卡片让主控转手会制造第三份状态载体。Harness 的第二阶段消息只承载角色 registration、routing 与当前 assignment delta。
