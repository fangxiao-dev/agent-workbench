...
它用独立构造的 fixture 覆盖五条自检判据、`SYNC STALE` 的错误信息完整性、决策与停滞的退出码优先级。**这份 fixture 刻意不复用 `ledger.py` 自身的解析逻辑**——用被测代码的假设去造测试数据，测不出假设本身是错的。（初版轮询契约就是这样漏掉了"rollout 只记录打印内容"这个事实。）
...
| `n` 等于 registry 中 **children** 数量 | `targets <n> != registry children <m>` |
| `polls` 每个元素至少含 `id` 与 `status` 两个键 | `poll entry shape altered` |
