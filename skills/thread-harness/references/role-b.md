...
按 `/impl-package:impl-package` 干活；调查、实现、修复和验证由 `$dispatcher` 形成执行合同，`/impl-package:do-review` 与 `/impl-package:verification-before-completion` 仍按各自合同执行；H1 / H2 / H4 同样适用。

子线执行协作由 `$dispatcher` 决定；本角色不另设 mode。
