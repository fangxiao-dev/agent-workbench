...
**Recommend /plan-review only as an independent plan-stage review** when this CEO review materially expanded scope, changed architectural direction, or accepted scope expansions. Do not infer its freshness from the gstack dashboard because `plan-review` keeps its own review state.
...
**If both plan and design review are needed, recommend plan review first**, then design review. The pre-landing `/review` gate remains later in the implementation workflow.
