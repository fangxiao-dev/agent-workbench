...
动作可二元判定只是必要条件，不是充分条件。派发前还必须通过**不可约分检查**：

- 列出本次动作包含的可观察子结果。
- 若任一子结果能够在其余工作完成前，独立改变、停止或重新排序主控的下一次派发，它就是独立可消费结果——此时继续切分，由主控消费局部结果后综合，不把跨部分的取证与综合包装成一次派发。
- 只有辅助调查、实现和验证都不产生上述独立结果时，才可以留在同一个 baby step。

多个材料族、逐项结论或完整候选表是需要检查的信号，不是按关键词机械拒绝。这个检查不按文件数量、步骤数、内部命令数或检索范围设硬上限：为一个窄问题在整个仓库检索时，检索范围宽仍可以是一个动作；为一个 coherent outcome 修改多个紧密相关文件时，跨文件也仍可以是一个动作，只要其中没有任何一部分能先独立改变、停止或重新排序后续调度。
