...
2. 为每个已解锁 Topic 选择当前最大 coherent step，把互不依赖且资源隔离的步骤组成当前批次并 fan out。文件 ownership 交叉时先由 SDD 判断能否用隔离 worktree 分开。共享操作的合并与复用只是调度优化，不是 dependency；只有不延迟已解锁的独立动作时才合并，否则先执行当前合格步骤并在 return 后重扫。
