...
def load_method_activation(method_root: Path | str) -> dict[str, str]:
    """Return the public Skill bundle's portable Git identity and exact commit."""
    root = Path(method_root).resolve()
    required = (
        root / "skills" / "backfill-stable-docs" / "SKILL.md",
        root / "skills" / "impl-package" / "SKILL.md",
    )
    missing = [str(path) for path in required if not path.is_file()]
    if missing:
        raise ConfigError(
            "method root must contain backfill-stable-docs and impl-package: "
            + ", ".join(missing)
        )
    try:
        ...
        if Path(top_level).resolve() != root:
            raise ConfigError(f"method root must be the Git top level: {root}")
        ...
        raise ConfigError(f"unable to resolve method activation: {detail}") from error
    repository = remote.strip().replace("\\", "/")
    if repository.endswith(".git"):
        repository = repository[:-4]
    if ":" in repository and not repository.startswith(("http://", "https://")):
        repository = repository.split(":", 1)[1]
    if "://" in repository:
        repository = repository.split("://", 1)[1].split("/", 1)[-1]
    repository = repository.strip("/").lower()
    if PORTABLE_REPOSITORY_RE.fullmatch(repository) is None:
        raise ConfigError("method origin must resolve to portable owner/repository identity")
