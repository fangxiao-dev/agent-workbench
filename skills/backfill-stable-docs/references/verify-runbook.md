...
- inventory/Markdown 汇总必须分别给出 `indexed`、`legacy-heading`、`mismatch`、`manual` 四类计数；`mismatch`/`manual` 都进入人工复核且不得因 open pending reference 隐藏。没有 `gate.md` 保持 open/no-verdict，不计入四类。
