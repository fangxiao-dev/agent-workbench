...
4. 补充做 gap-catching 扫描：枚举 gate ledger 有可信 terminal entry、相关实现 commit 已进入 `targetBranch`、但对应 `_pending.md` 里没有登记的 package。机械候选只接受 `gateRecognition=indexed|legacy-heading` 且 `gateResolution=pass|fail|defer`；`fail` 仍是 terminal，`blocked` 不是。只有这类 package 才需要重新读 `design.md`、`spec.md`、`plan.md`、review、handoff、tickets 和 Git commit 去发现候选；`findings.md` 仅在 design/spec 明确引用或出现 evidence gap / authority conflict 时作为 supplemental evidence。
   - Gate 识别报告四类：content-bound finalized index 完整核验通过为 `indexed`；整个 package 不存在 runtime-state 时才允许解析旧 heading 为 `legacy-heading`；runtime-state 存在但缺 entry、字段/hash 不符、陈旧或损坏为 `mismatch`；无 runtime-state 且 heading 不可解析为 `manual`。`mismatch`/`manual` 均不提供可信 resolution，必须 fail closed 且不能因为 package 已被 pending 引用而从人工复核汇总隐藏。agent 仍须在同一轮 audit 里读完这些证据并给出正常分类；只有证据本身矛盾或缺失才升级 owner 裁决。没有 `gate.md` 表示 open/no-verdict，不属于四类 recognition。
