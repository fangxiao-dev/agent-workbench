...
1. 读取 decision/spec 当前 revision、current attempt plan、gate.md 最新 entry 与所需 module knowledge/code。
...
4. `dag=true` 时从最小 Task graph 和 runtime state 恢复 Task；默认不创建 progress。仅 BLOCKED、handoff、retry 或并行派发 Task 使用 `tasks/Tn-progress.md`。
...
4. 固定 comparison point 后，按当前 diff 与 contract impact 运行 Ticket 正式 review；Task 局部验证或提前风险检查不能替代正式 review/acceptance。
...
3. 固定 comparison point 和 plan ER anchor，分流 execution findings，完成必要 D/S revision 与 re-gate。
