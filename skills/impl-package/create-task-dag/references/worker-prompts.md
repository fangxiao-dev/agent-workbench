...
正常返回可使用 `PENDING`、`RUNNING`、`DONE` 或 `BLOCKED`。`DONE` 应附局部改动与验证 evidence，但仅表示可以交回 Working Branch owner 集成。发现共享 seam 不能擅自编辑非属地范围，也不能返回 `NEEDS_SEAM`；以 BLOCKED 报告最小原因、建议动作和影响 Ticket。仅满足条件时才更新 `tasks/Tn-progress.md`；不创建独立 Ticket progress artifact、不维护第二套 acceptance 状态，也不直接编辑 Ticket，除非主 session 明确委托。
