...
- 旧 audit schema 不再被 verifier 兼容读取；发现旧版本时先走 contract preflight 升级 package 和 audit 输入，再重新生成当前 `contractVersion: "3.1"` 记录。Git 负责历史 provenance，不在 audit JSON 内复制迁移日志。
