...
- inventory/Markdown 汇总必须分别给出 `current` 与 contract drift advisory package 数量，以及 `indexed`、`mismatch`、`manual` 三类 Gate 计数；对 indexed entry 报告是否适用于当前 D/S/P，合法历史 entry 保持 `indexed` 但没有当前 resolution。contract preflight 不因缺失或过期 package 阻断 verify，也不自动升级 package；非 current package 的 Gate 不作为兼容输入，不能进入 gap-catching/retirement。inventory 的 `gapCatchingStructuralCandidates` 仅表示尚未核验 Git reachability 的结构清单，verify 不得把它直接当 audit 的真实 `gapCatchingCandidates`。`mismatch`、`manual` 和 contract drift 都进入人工复核且不得因 open pending reference 隐藏。没有 `gate.md`，或只有空 ledger 且尚无 allocation/entry，保持 open/no-verdict，不计入三类。
