...
- inventory/Markdown 汇总必须分别给出 `indexed`、`mismatch`、`manual` 三类计数，并对 indexed entry 报告是否适用于当前 D/S/P；合法历史 entry 保持 `indexed` 但没有当前 resolution。contract preflight 在 verify 前阻断缺失或过期 package；旧格式不作为兼容输入。inventory 的 `gapCatchingStructuralCandidates` 仅表示尚未核验 Git reachability 的结构清单，verify 不得把它直接当 audit 的真实 `gapCatchingCandidates`。`mismatch`/`manual` 都进入人工复核且不得因 open pending reference 隐藏。没有 `gate.md`，或只有空 ledger 且尚无 allocation/entry，保持 open/no-verdict，不计入三类。
