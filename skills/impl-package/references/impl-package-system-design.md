...
- req-align 在 Decision 开始前生成并记录 package-id，同时记录 topic slug；创建后两者均不可改写。若精确目录名已存在，保留同一日期和 topic slug 并追加 `-02`、`-03`…顺序后缀，直到唯一。
...
| **dag**（执行依赖/协调） | 需要显式表示多 owner 协作、task 间非平凡依赖，或跨 slice seam 协调 | 当前 attempt DAG | 不建立 task checklist；仅实际 Task 因 blocker、handoff、重试或并行派发需要时创建 progress |

两个开关任意组合合法：都无（spec+plan 直接执行）、只 dag、只 tickets、两者都有。Task 与 Ticket 解耦——Task 可横切多个 Ticket；共享 seam 在 Working Branch owner integration 中处理，不形成单独的 Task 类型。
...
- `tasks/Tn-handoff.md` 只在实际 Task 因 blocker、handoff、重试或并行派发需要继续时创建，不随开关自动创建。
- 跨 session 恢复本身不 earn dag：根 `progress.md` 恢复当前 Attempt 与两条状态轴，`execution-records/index.md` 指向历史，必要事实进入 Attempt ER 或 Task handoff。若执行拓扑无法由 plan/tickets 清晰重建，则因“执行依赖需要显式化”earn dag，而不是因“跨 session”earn dag。
...
```text
1 对齐与调研  req-align（通用化后）→ decision.md + 首批 execution findings + owner decisions
...
- **Stage 3/4 计划拆解**：attempt plan 就绪后进入一个 earn-gated decomposition stage；`tickets=true` 时由 to-tickets 先形成完整 Draft Ticket 集合，`dag=true` 时由 create-task-dag 基于 plan 与 Draft Tickets（或仅 plan）生成 DAG，随后联合校验覆盖、依赖、ownership、acceptance/evidence、gate 与 revision binding，并由 owner 一次 review/approval。plan 保存执行策略、具体 migration 操作、Planned Verification 与到 `progress.md`/ER index 的链接，不复制 ticket/task/ER 正文；未获 bundle approval 不进入执行或 preflight。
- **职责仍分层、治理合并**：ticket 列表（delivery slices + 带阻塞语义的静态 `blocked by` 依赖标注）归 to-tickets fork；task DAG（横向的 execution ownership、依赖和多对多 Ticket contribution）归 create-task-dag。两个工具保留独立 artifact ownership，但不再各自设置批准门；输入过宽路由回计划拆解的 to-tickets draft，不能从单个 ticket 猜测跨 ticket seam contract。
...
- **Gate ledger**：package 只保留一个 `gate.md`，每次 evaluation 在顶部插入 `<attempt-id>-G<n>` 摘要。blocked→pass 通过新 entry 与 `Supersedes` 表达；旧块不改。完整验证过程放在对应 Attempt 的 sealed Execution Record，gate 只保存 revision/comparison point/evidence anchor/verdict 与 Durable Deltas。terminal verdict 先保留 G id 并完成 Stage 7，再一次性插入不可变 entry；不写临时 entry 后原地补字段。
- **Revision-blob binding**：D/S/P revision 号是人类好念的别名，机器校验身份保存在 package-local `.impl-package/revision-bindings.json` sidecar 的 artifact path + Git blob OID 中。artifact 不保存自身 hash，因此 sidecar 可与最终 artifact 在同一 commit 中稳定绑定；restore/gate evaluation 用 `git rev-parse HEAD:<path>` 机械复核。可证明零语义影响的 editorial correction 只更新同 alias binding evidence，任何语义不明或实质 contract 变化仍升级 revision 并走对应 Gate。sidecar 不进入 owner-facing 交付主线，Markdown 直接呈现 revision set 与校验结论。机制细节见 composition-contract §2。
- **execution findings 分流阻断**：`execution-findings.md` 在 pass/fail/defer 前必须判断对 decision/spec/plan/Durable Deltas 的影响；分流后原始发现可继续保留为 package-local provenance。它不是第二份合同或临时待办。blocked entry 不受此约束。见 composition-contract §6。
- **investigations earned-only**：`investigations/<topic>.md` 只承载原始 ideas、调查过程、候选假设、实验材料与参考笔记，默认无 authority，不创建空目录，不绑定 revision、不进入 runtime 或 projection；正式 decision/spec 必须自足，并仅在需要 provenance 时单向链接 investigation。
...
| `plan.md` / patch plan | impl-planning | P revision；链接 progress/ER index | 长期 contract、ticket/task runtime status、ER 正文、通用 checklist 副本 |
...
| `progress.md` | dev-with-track/state CLI | machine projection | 手工状态、历史日志、Task/Ticket ownership |
| `execution-records/<attempt>.md` + `index.md` | dev-with-track 主 session/state CLI | `er-add` append | Ticket/Task ownership、长期 contract、第二套 acceptance state |
| `tasks/Tn-handoff.md` | Task worker/main session | 条件化局部交接 | 迷你 spec、重复计划、package acceptance 结论 |
| `execution-findings.md` | dev-with-track（格式与分流） | **各阶段均可 append** | 第二份 decision/spec、临时待办、最终验证证据 |
| `investigations/<topic>.md` | 产生调查材料的阶段 | 原始调查可继续补充 | authority、runtime state、revision binding、machine projection、正式文档 backlink |
| `gate.md` | dev-with-track | 顶部新增不可变 evaluation entry | 完整 checklist、旧 entry 修改、长期 contract |

Artifact ownership 仍分别归 to-tickets 与 create-task-dag；计划拆解只合并治理门，不新增 manifest、schema、CLI 或可写状态 SoT。

## Composition 状态机与计划拆解跨层契约

本节是各 skill 的共同规范源；skill 可引用、不可各自重定义以下规则。

### 计划拆解 bundle

计划拆解是一个 earn-gated 阶段，不把 Ticket 与 DAG 分成两个审批事实源：`tickets=true` 时先由 to-tickets 形成完整 Draft Ticket 集合，`dag=true` 时再由 create-task-dag 基于 plan 与 Draft Tickets（或仅 plan）生成 DAG；随后联合校验覆盖、依赖、ownership、acceptance/evidence、gate 与 revision binding，并由 owner 对当前 earned artifacts 一次 review/approval。Ticket 变化若触及 DAG dependency、contribution、ownership 或顺序，必须重新生成或修订受影响 DAG；未获 bundle approval 不进入 execution/preflight。

面向 owner 的进度可报告为 `drafting`、`ready-for-review`、`approved`、`in-progress`、`completed`；其中前三者描述 bundle readiness，后两者继续由既有 Attempt、Task、Ticket acceptance 和 gate 派生。它们不是新的 sidecar 字段、CLI 或可写状态 SoT，也不替代 Draft/Active/Frozen lifecycle；四种 Composition 仍按下表保留，各自只创建 earned artifact。

### 四种组合的 canonical status home

| Composition | 执行状态 | ticket 验收状态 |
| --- | --- | --- |
| 无 tickets、无 dag | 无 task artifact；根 `progress.md` 恢复 Attempt，历史进入 Attempt ER 或 handoff | spec AC + Attempt ER + gate entry |
| 仅 tickets | runtime-state 是 Ticket 状态 SoT，ticket 文件为投影，根 `progress.md` 展示 acceptance | Ticket Runtime Acceptance projection + Attempt ER |
| 仅 dag | current DAG/runtime state 索引 Task，根 `progress.md` 展示执行轴，handoff 按需创建 | spec AC + Attempt ER + gate entry |
| tickets + dag | `dag.md` 与 ticket 文件各自投影 runtime state，根 `progress.md` 分轴展示 | Ticket Runtime Acceptance projection + Attempt ER |
...
- 联合 review/发布前校验无环、无缺失引用。多个 actionable 单元按 ticket/task 文档顺序稳定选择，不做自动派工。
- 上游返工或验收重开时，只有依赖受影响产物且证据可能失效的下游回退为 `NEEDS-REVALIDATION`；执行者按 impact subset 定向重验证，未受影响项可批量确认，不自动扩散到整条链。
- restore 时先对账 artifact 状态与证据；不一致时以可核实证据为准，并修正 canonical status home 后再选下一单元。

### Task 到 ticket acceptance 的 many-to-many 追踪

task 与 ticket 不建立包含关系，但以多对多 contribution mapping 关联。只有 `dag=true` 时存在结构化 task，`dag.md` 为每个 task 记录 primary ownership、known depends on、contributes-to tickets 和 known seam/risk；不要求 `enables`、Task→AC 或完整 task contract。no-DAG attempt 不为追踪而制造 task checklist；其 AC 覆盖由 spec evidence owner、Planned Verification 与 Execution Record 共同证明。
...
- integration responsibility：现有 Working Branch owner 在并行 Task 返回、出现 BLOCKED 或 Ticket 验收前完成 integration step；这不是新角色或 artifact。
- seam evidence 在 Working Branch owner integration 后映射回受影响 Ticket AC；不创建独立 seam acceptance owner。
- 任一 seam acceptance 未通过时，所有依赖该 seam 的 ticket 不得关闭。
- Task 只做局部验证；正式 Ticket review 检查完整 implementation 的 contract fidelity，不以某一个 Task 为审查边界。

### Attempt Composition 修订

允许活动 attempt 通过新 P revision 修订 Composition，不允许双写。记录 previous/new、理由、时间与 artifact relocation；创建或退休当前 attempt 的 tickets/DAG 状态来源，并使旧计划拆解 bundle approval 按影响失效。该修订不改变 D/S revision，除非同时发现 contract drift。

P revision 升级后，已创建但仍声明旧 P 号的 ticket/DAG 视为 `NEEDS-REVALIDATION`；先从 P delta 识别受影响 subset。受影响项定向核对或重新生成，未受影响项可批量确认并机械更新字段；restore 不把 revision mismatch 等同于全部内容失效。

### Stage 7 完整关闭契约与可选 backfill

durable delta 的 canonical 捕获面是 **gate 最新 evaluation entry 的 Durable Deltas → `_pending.md`**，不在 `spec.md` 维护常青 `Stable Doc Backfill Map`（那会与下游 backfill 体系形成三重登记，违反不双重维护）。`decision.md` 的 Backfill Candidates 只是非约束调研提示，正式捕获在 gate 关闭那一刻发生，不需要在执行期归并进 spec。去重键 `<destination>|<delta-id>` 落在 `_pending.md` 与 report 侧。
...
| `to-tickets` | **本地 fork**（保留名，registry 标注"已本地分叉，上游更新人工 diff"）：加 draft/publish 双模式（内部默认 draft；publish 仅作为联合 bundle 的发布动作）、runner-neutral handoff、删除 /implement 绑定；保留 tracer bullet、带类型的静态 blocking edges、wide-refactor expand–contract；生成完整 Draft Ticket 集合并参与联合校验，不再设置独立批准门 |
...
| 1 | req-align 两道对等必过门 + decision/spec ownership + 厚 spec 模板 | 两门都有可检验必过标准（Decision 步骤不可跳）；spec 模板含八节含 Error Boundaries；decision 护栏语句在位 |
...
| 记录 | 维度 | 触发 |
| --- | --- | --- |
| `tasks/Tn-handoff.md` | task（执行单元） | 仅 BLOCKED、跨 session handoff、重试或主 session 分发并行 subagent 时创建；只记录 blocker/原因、已有证据、下一动作和影响 Ticket |
...
- 根 `progress.md` 是所有 Composition 的统一当前恢复视图；Ticket 的 AC、evidence 和 Runtime Acceptance Status 仍是长期验收事实，Task handoff 只保存局部交接，均不参与 acceptance 或 dependency release。
...
5. revision 漂移防护：D/S/P revision 号通过外部 registry 绑定 Git blob、P 号驱动 ticket/DAG 的 NEEDS-REVALIDATION、execution findings 分流阻断、Module Knowledge Watermark，四条均已确认并落入 composition-contract（对抗审视后补，见 §2/§3/§6/§1）。
6. 风险驱动质检：`grill-me-smartly` 只在明确请求或高不确定性/高风险信号时运行，`grilling` 留作可选深度对抗（已确认）。二者都是既有 skill，req-align 只负责触发与结果分流，不复制或重定义其内部机制。
