...
2. terminal Gate 前分流：行为合同回 Decision/Spec；长期项目知识登记为带可读 delta-id 的 Durable Delta 与 truth pointer/stub；没有增量时记录明确原因。需要人工提前跟踪时才写入可选 `_pending.md`。
3. `backfill-stable-docs` 从 Gate Durable Deltas 和可选 pending registry 建立 item inventory；terminal Gate 的 comparison commit 已进入 targetBranch 且 item 不在 `records.done` 时形成 gap-catching。pending 不抑制 gap-catching。
