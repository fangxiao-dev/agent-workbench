...
4. Update the existing Agent-skills block in `AGENTS.md` or `CLAUDE.md` in place. State that `$triage` proposes before writing and `$issue-reporter` is read-only.
...
Report the local files changed, configured alias mappings, and whether GitHub label materialization still needs a separate confirmed `$triage` proposal.
