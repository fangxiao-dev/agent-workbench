...
MANAGED_AGENT_FILE_MARKER = "# managed-by = agent-workbench:codex-setup"
...
def _agent_inventory() -> tuple[list[Path], list[Path]]:
    ...
    for item in sorted(root.iterdir(), key=lambda path: path.name.lower()):
        if item.name.startswith("."):
            continue
        if item.is_dir():
            directories.append(item.resolve())
...
def _expected_agent_sources() -> list[Path]:
    return _agent_inventory()[0]


def _expected_agent_files() -> list[Path]:
    return _agent_inventory()[1]
...
def _audit_role(
    ...
    *,
    kind: str = "role",
    expected_path: str = "generated by install_codex_agents.py",
) -> AuditItem:
    ...
    try:
        ...
        return _blocked(kind, name, f"cannot read {destination}: {exc}")
...
def _stale_agent_files(
    ...
    results: list[AuditItem] = []
    try:
        children = sorted(installed_root.iterdir(), key=lambda path: path.name.lower())
    except OSError as exc:
        return [_blocked("agent-file", installed_root.name, f"cannot enumerate {installed_root}: {exc}")]
    marker = f"{MANAGED_AGENT_FILE_MARKER}\n".encode("utf-8")
    for child in children:
        if child.name in expected_names or child.suffix.lower() != ".toml":
            continue
        if link_skill.is_link(child) or not child.is_file():
            continue
        try:
            content = child.read_bytes()
        except OSError as exc:
            results.append(_blocked("agent-file", child.name, f"cannot read {child}: {exc}"))
            continue
        ...
        results.append(
            AuditItem(
                ...
                name=child.name,
                status="INSTALL-MISMATCH",
                expected_path="removed from current agents/ inventory",
                actual_path=str(child),
                note="managed source was removed; target was retained",
            )
        )
    return results
...
def audit_codex(codex_home: Path) -> AuditResult:
    ...
    try:
        agent_sources, agent_files = _agent_inventory()
    except OSError as exc:
        ...
        for source in agent_files:
            try:
                ...
                item = _blocked("agent-file", source.name, f"cannot read {source}: {exc}")
            else:
                item = _audit_role(
                    source.name,
                    content,
                    MANAGED_AGENT_FILE_MARKER,
                    agents_dir / source.name,
                    kind="agent-file",
                    expected_path=str(source),
                )
            items.append(item)
        ...
        stale_agent_files = _stale_agent_files(agents_dir, {source.name for source in agent_files})
        items.extend(stale_agent_files)
        operational_error = operational_error or any(
            item.status == "BLOCKED" for item in stale_agent_files
        )
...
def _link_ancestor(path: Path) -> Path | None:
    current = link_skill.abspath_nofollow(path)
    while True:
        if link_skill.is_link(current):
            return current
...
def _write_agent_file_atomically(source: Path, destination: Path) -> None:
    unsafe = _link_ancestor(destination)
    if unsafe is not None:
        raise OSError(f"refusing to write through symlink or reparse point: {unsafe}")
...
def _apply_agent_files(
    ...
    *,
    ...
    actions: list[str] = []
    try:
        sources = _expected_agent_files()
    except OSError as exc:
        return False, [f"FAIL agent inventory — {exc}"]
    ...
    for source in sources:
        item = states.get(source.name)
        if item is None:
            return False, [f"FAIL missing audit item for agent file `{source.name}`"]
        if item.status == "MATCH":
            actions.append(f"SKIP agent file `{source.name}` (already matches)")
            continue
        if item.status == "INSTALL-MISMATCH" and not adopt_agents:
            return False, [
                f"BLOCKED agent file `{source.name}` is not managed; "
                "rerun apply with `--adopt-agents`"
            ]
        if item.status == "BLOCKED":
            return False, [f"BLOCKED agent file `{source.name}` audit failed"]
        try:
            _write_agent_file_atomically(source, codex_home / "agents" / source.name)
        except OSError as exc:
            return False, [f"FAIL agent file `{source.name}` — {exc}"]
        actions.append(f"UPDATED agent file `{source.name}`")
    return True, actions
...
def apply_codex(
    ...
    *,
    ...
    actions: list[str] = []
    env = _codex_env(codex_home)
    unmanaged_agent_files = [
        item
        for item in pre.items
        if item.kind == "agent-file" and item.status == "INSTALL-MISMATCH"
    ]
    if unmanaged_agent_files and not adopt_agents:
        names = ", ".join(f"`{item.name}`" for item in unmanaged_agent_files)
        return [
            f"BLOCKED unmanaged agent files: {names}; "
            "rerun apply with `--adopt-agents`"
        ], False
    ...
    if any(item.kind == "agent-file" and item.status != "MATCH" for item in pre.items):
        ok, agent_file_actions = _apply_agent_files(
            pre,
            codex_home,
            adopt_agents=adopt_agents,
        )
        actions.extend(agent_file_actions)
        if not ok:
            return actions, False
...
def build_parser() -> argparse.ArgumentParser:
    ...
    apply_parser.add_argument(
        "--adopt-agents",
        action="store_true",
        help="adopt existing unmanaged top-level agent TOML files from the repository inventory",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        actions, apply_ok = apply_codex(pre, codex_home, adopt_agents=args.adopt_agents)
