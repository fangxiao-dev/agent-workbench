...
**2026-09-08 E1 最后残留修复**：撤销此前“缺少 Attempt 行按 legacy 匹配”的假设。`_parse_gate` 将缺少 Attempt 的 Gate 标为无效并清除 verdict，归属判据返回 unknown；JSON 与 SessionStart capsule 不再把该 verdict 当成当前终态。52 个原有最小 Gate fixture 补齐所属 Attempt，替换默认匹配断言，并新增实际 SessionStart 回归覆盖缺少 Attempt 与旧 Attempt 两种情况。`python -B -m pytest tests/test_situation_render.py tests/test_impl_package_hooks.py -q -p no:cacheprovider`：113 passed；`git diff --check` 通过。本次为修复及聚焦验证，未追加全仓测试或独立终审声明。
